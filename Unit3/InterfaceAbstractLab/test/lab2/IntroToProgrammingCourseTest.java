...
public class IntroToProgrammingCourseTest extends TestCase {
    ...
    public void testGetPrerequisites() {
        assertEquals("none", itp.getPrerequisites());
    }
    ...
    public void testSetPrerequisites() {
        try{
            itp.setPrerequisites("English");
        }catch (IllegalArgumentException e){
            assertEquals("Error: intro courses cannot have prerequisites",e.getMessage());
        }
    }

    @Test
    public void testToString() {
        assertEquals("test is course #123. It is worth 1.0 credits. There are no prerequisites.", itp.toString());
    }
}